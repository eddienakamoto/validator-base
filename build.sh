...
if [[ ! "$platforms" =~ ^(linux/amd64|linux/arm64|linux/amd64,linux/arm64|linux/arm64,linux/amd64)$ ]]; then
    echo "Invalid platforms: $platforms. Must be either linux/amd64, linux/arm64, or both."
    print_usage
    exit 1
fi
